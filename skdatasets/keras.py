...
datasets = {'boston_housing': {'loader': boston_housing.load_data, 'pixel_max': None},
...
def load(name, return_X_y=False):
    ...
    if datasets[name]['pixel_max'] is not None:
        X = X.reshape([X.shape[0], np.prod(X.shape[1:])]) / datasets[name]['pixel_max']
        X_test = X_test.reshape([X_test.shape[0], np.prod(X_test.shape[1:])]) / datasets[name]['pixel_max']
